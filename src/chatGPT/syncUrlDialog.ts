...
export function showSyncUrlDialog(): Promise<string | null> {
  ...
  let useSmallOverlay = getOverlaySize();
  let buttonColor = getButtonBgColor();
      ...
      <div id="dialog-content" style="padding-top: 10px;">
        ...
        <div id="options-content" style="display: none;">
          <div style="margin-top: 20px;">
            <label>
              <input type="checkbox" id="use-small-overlay" style="margin-right: 8px;" ${useSmallOverlay ? "checked" : ""}>
              Use small overlay
            </label>
          </div>
          <div style="margin-top: 20px;">
  ...
  return new Promise<string | null>((resolve) => {
    ...
    optionsTab.onclick = () => {
      // Update the input fields when the Options tab is opened
      (document.getElementById("use-small-overlay") as HTMLInputElement).checked = getOverlaySize();
      (document.getElementById("bg-color-input") as HTMLInputElement).value = getButtonBgColor();
    ...
    document.getElementById("options-ok")!.onclick = () => {
      useSmallOverlay = (document.getElementById("use-small-overlay") as HTMLInputElement).checked;
      buttonColor = (document.getElementById("bg-color-input") as HTMLInputElement).value || "green";
      setOverlaySize(useSmallOverlay);
