export function attachLinks() {
  ...
  codeBlocks.forEach((block) => {
    if (!(block as any).attachedCodespinLink) {
      ...
      if (
        ...
        copyButton.parentElement.parentElement.insertBefore(
          container,
          copyButton.parentElement
        );
      }
      (block as any).attachedCodespinLink = true;
    }
  });
}

function onCopyClick(this: GlobalEventHandlers, ev: MouseEvent): void {
  const codeElement = findClosestPreElement(ev.target as HTMLAnchorElement);
  console.log(codeElement?.innerText);
}

function findClosestPreElement(element: HTMLElement | null) {
  // Check if the input is a valid DOM element
  if (!(element instanceof Element)) {
    throw new Error("Input is not a valid DOM element.");
  }

  // Traverse up the DOM tree to find the closest <pre> element
  while (element) {
    if (element.tagName.toLowerCase() === "pre") {
      // Find the <code> element within this <pre> element
  ...
  // Return null if no <pre> element or <code> element within it is found
  return null;
}

setTimeout(attachLinks, 1000);
setInterval(attachLinks, 3000);
