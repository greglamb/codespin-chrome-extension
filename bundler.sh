...
if [ -z "$1" ]; then
  echo "Usage: bundler.sh <npm_package_name> --out <output_directory> [--esm] [--ts] [--entry <entry_file>]"
  exit 1
fi
...
TS=false
OUT_DIR=""
ENTRY_FILE="./index.js" # Default entry file
...
# Check if the specified entry file exists, otherwise use the default entry file
ENTRY_PATH="$TEMP_DIR/node_modules/$PACKAGE_NAME/$ENTRY_FILE"
if [ ! -f "$ENTRY_PATH" ]; then
  echo "Error: Entry file not found: $ENTRY_PATH"
  exit 1
fi
